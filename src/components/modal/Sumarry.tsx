        ...
        <DialogContent className="sm:max-w-[600px] max-h-[80vh] overflow-y-auto">
          <div className="grid gap-4 py-4">
            <h2 className="text-xl font-semibold">
              {summary?.data?.videoInfo.name}
            </h2>
            <div className="space-y-4">
              {summary?.data?.transcripts &&
                Object.values(summary.data.transcripts).map(
                  (transcriptGroup: any, idx: number) =>
                    (
                      transcriptGroup["custom"] ?? transcriptGroup["default"]
                    )?.map((item: any) => (
                      <TranscriptItem
                        key={item.start + "-" + idx}
                        item={item}
              ...
              Object.values(summary.data.transcripts).map(
                (transcriptGroup: any, idx: number) =>
                  (
                    ...
                    <TranscriptItem key={item.start + "-" + idx} item={item} />
                  ))
              )}
          </div>
        </div>
  ...
  return (
    <div className="px-4 py-3 border-b last:border-b-0 hover:bg-muted/20">
      <div className="flex items-start justify-between">
        <div className="min-w-[50px] font-mono text-sm text-blue-500">
          {item.start}
        </div>
      ...
      {isExpanded && (
        <div className="mt-2 pl-[50px] text-sm text-muted-foreground pt-2">
          {item.text}
        </div>
      )}
    </div>
